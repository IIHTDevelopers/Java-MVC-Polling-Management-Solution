...
public class PollControllerTest {
    ...
    public void testPollControllerUpdatePolls() throws Exception {
        ...
        MvcResult result = this.mockMvc.perform(get("/poll/update")
                .param("pollId", poll.getPollId().toString())).andReturn();
        yakshaAssert(currentTest(), result.getModelAndView().getViewName().contentEquals("updatePolls") ? true : false, businessTestFile);
    ...
    public void testPollControllerAddPollHandleForm() throws Exception {
        ...
        MvcResult result = this.mockMvc.perform(post("/poll/handleForm")
                        .flashAttr("poll", poll))
                .andReturn();
        yakshaAssert(currentTest(), result.getModelAndView().getViewName().contentEquals("redirect:/poll/list") ? true : false, businessTestFile);
    }
